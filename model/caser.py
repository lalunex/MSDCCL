...
class Caser(SequentialRecommender):
    ...
    def __init__(self, config, dataset):
        ...
        if config['load_pre_train_emb'] is not None and config['load_pre_train_emb']:
            ...
            self.item_embedding = nn.Embedding.from_pretrained(checkpoint['state_dict']['item_embedding.weight'],
                                                           freeze=False)
            self.user_embedding = nn.Embedding.from_pretrained(checkpoint['state_dict']['user_embedding.weight'],
                                                           freeze=False)
    ...
    def forward(self, user, item_seq_emb):
        # Embedding Look-up
    ...
    def calculate_loss(self, interaction):
        ...
        seq_output = self.forward(user, self.item_embedding(item_seq))
